...
fn coerce_to_serial(arg: &ArgumentHandle) -> Result<f64, ExcelError> {
    ...
    match v {
        ...
        LiteralValue::Text(s) => s.parse::<f64>().map_err(|_| {
            ExcelError::new_value().with_message("Not a valid number")
        }),
...
impl Function for WeekdayFn {
    ...
    fn arg_schema(&self) -> &'static [ArgSchema] {
        ...
        static SCHEMA: LazyLock<Vec<ArgSchema>> = LazyLock::new(|| {
            vec![ArgSchema::number_lenient_scalar(), ArgSchema::number_lenient_scalar()]
        });
        &SCHEMA[..]
    }
    fn eval<'a, 'b, 'c>(
        ...
        let result = match return_type {
            ...
            11 => match weekday { // Mon=1..Sun=7
                Weekday::Mon => 1, Weekday::Tue => 2, Weekday::Wed => 3,
                Weekday::Thu => 4, Weekday::Fri => 5, Weekday::Sat => 6, Weekday::Sun => 7,
            },
            12 => match weekday { // Tue=1..Mon=7
                Weekday::Tue => 1, Weekday::Wed => 2, Weekday::Thu => 3,
                Weekday::Fri => 4, Weekday::Sat => 5, Weekday::Sun => 6, Weekday::Mon => 7,
            },
            13 => match weekday { // Wed=1..Tue=7
                Weekday::Wed => 1, Weekday::Thu => 2, Weekday::Fri => 3,
                Weekday::Sat => 4, Weekday::Sun => 5, Weekday::Mon => 6, Weekday::Tue => 7,
            },
            14 => match weekday { // Thu=1..Wed=7
                Weekday::Thu => 1, Weekday::Fri => 2, Weekday::Sat => 3,
                Weekday::Sun => 4, Weekday::Mon => 5, Weekday::Tue => 6, Weekday::Wed => 7,
            },
            15 => match weekday { // Fri=1..Thu=7
                Weekday::Fri => 1, Weekday::Sat => 2, Weekday::Sun => 3,
                Weekday::Mon => 4, Weekday::Tue => 5, Weekday::Wed => 6, Weekday::Thu => 7,
            },
            16 => match weekday { // Sat=1..Fri=7
                Weekday::Sat => 1, Weekday::Sun => 2, Weekday::Mon => 3,
                Weekday::Tue => 4, Weekday::Wed => 5, Weekday::Thu => 6, Weekday::Fri => 7,
            },
            17 => match weekday { // Sun=1..Sat=7
                Weekday::Sun => 1, Weekday::Mon => 2, Weekday::Tue => 3,
                Weekday::Wed => 4, Weekday::Thu => 5, Weekday::Fri => 6, Weekday::Sat => 7,
            },
            _ => return Ok(CalcValue::Scalar(LiteralValue::Error(ExcelError::new_num()))),
        };
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let week_starts = match return_type {
            ...
            21 => {
                ...
                return Ok(CalcValue::Scalar(LiteralValue::Int(date.iso_week().week() as i64)));
            }
            _ => return Ok(CalcValue::Scalar(LiteralValue::Error(ExcelError::new_num()))),
        };
...
impl Function for DatedifFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let unit = match args[2].value()?.into_literal() {
            ...
            _ => return Ok(CalcValue::Scalar(LiteralValue::Error(ExcelError::new_value()))),
        };
            ...
            "MD" => {
                ...
                if days < 0 {
                    ...
                    let days_in_prev_month = (NaiveDate::from_ymd_opt(
                        if prev_month.month() == 12 { prev_month.year() + 1 } else { prev_month.year() },
                        if prev_month.month() == 12 { 1 } else { prev_month.month() + 1 },
                        1
                    ).unwrap() - prev_month).num_days();
...
impl Function for WorkdayFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        Ok(CalcValue::Scalar(LiteralValue::Number(date_to_serial(&current))))
    }
}
...
mod tests {
    ...
    fn weekday_basic() {
        ...
        assert_eq!(
            f.dispatch(&[ArgumentHandle::new(&n, &ctx)], &ctx.function_context(None))
                .unwrap()
                .into_literal(),
            LiteralValue::Int(2)
        );
