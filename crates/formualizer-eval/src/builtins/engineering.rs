...
use super::utils::{coerce_num, ARG_NUM_LENIENT_TWO, ARG_ANY_TWO};
...
fn erf_approx(x: f64) -> f64 {
    ...
    if ax < 4.0 {
        ...
        return if x > 0.0 { 1.0 - erfc_val } else { erfc_val - 1.0 };
    }

    // For large x, erf(x) ≈ ±1
    let erfc_val = erfc_large(ax);
    if x > 0.0 { 1.0 - erfc_val } else { erfc_val - 1.0 }
}
...
static ARG_COMPLEX_THREE: std::sync::LazyLock<Vec<ArgSchema>> = std::sync::LazyLock::new(|| {
    vec![ArgSchema::any(), ArgSchema::any(), ArgSchema::any()]
});
