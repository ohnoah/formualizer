...
impl Function for AccrintFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let basis_int = if args.len() > 6 { coerce_num(&args[6])?.trunc() as i32 } else { 0 };
        let calc_method = if args.len() > 7 { coerce_num(&args[7])?.trunc() as i32 } else { 1 };
        ...
        let accrued_interest = if calc_method != 0 {
            ...
            let start_date = if prev_coupon < issue { issue } else { prev_coupon };
...
impl Function for AccrintmFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let basis_int = if args.len() > 4 { coerce_num(&args[4])?.trunc() as i32 } else { 0 };
...
impl Function for PriceFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let price = calculate_price(&settlement, &maturity, rate, yld, redemption, frequency, basis);
...
fn calculate_price(
    ...
    let dsn = if days_in_period > 0.0 { days_to_next / days_in_period } else { 0.0 };
    ...
    if n == 1 {
        ...
        let price = (redemption + coupon) / (1.0 + dsn * yld_per_period)
            - (1.0 - dsn) * coupon;
        price
    } else {
...
impl Function for YieldFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let yld = calculate_yield(&settlement, &maturity, rate, pr, redemption, frequency, basis);
...
fn calculate_yield(
    ...
    for _ in 0..MAX_ITER {
        ...
        let delta = 0.0001;
        let price_up = calculate_price(settlement, maturity, rate, yld + delta, redemption, frequency, basis);
    ...
    let final_price = calculate_price(settlement, maturity, rate, yld, redemption, frequency, basis);
