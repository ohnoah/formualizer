...
impl Function for SlnFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        if life == 0.0 {
            return Ok(CalcValue::Scalar(LiteralValue::Error(ExcelError::new_div())));
        }
...
impl Function for DbFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let month = if args.len() > 4 { coerce_num(&args[4])? } else { 12.0 };
        ...
        for p in 1..=period_int {
            let depreciation = if p == 1 {
                ...
                (value - total_depreciation - salvage).max(0.0).min(value - total_depreciation)
            } else {
...
impl Function for DdbFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let life = coerce_num(&args[2])?;
        let period = coerce_num(&args[3])?;
        let factor = if args.len() > 4 { coerce_num(&args[4])? } else { 2.0 };
