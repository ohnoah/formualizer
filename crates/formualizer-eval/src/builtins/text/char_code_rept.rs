...
impl Function for CharFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        Ok(CalcValue::Scalar(LiteralValue::Text(unicode_char.to_string())))
    }
}
...
impl Function for CodeFn {
    ...
    fn eval<'a, 'b, 'c>(
        ...
        let s = match v {
            ...
            LiteralValue::Empty => return Ok(CalcValue::Scalar(LiteralValue::Error(ExcelError::new_value()))),
            LiteralValue::Error(e) => return Ok(CalcValue::Scalar(LiteralValue::Error(e))),
...
mod tests {
    ...
    fn code_basic() {
        ...
        assert_eq!(
            f.dispatch(&[ArgumentHandle::new(&s, &ctx)], &ctx.function_context(None))
                .unwrap()
                .into_literal(),
            LiteralValue::Int(65)
        );
